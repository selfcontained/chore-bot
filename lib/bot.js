...
module.exports = function (controller, beepboop, log, config) {
  ...
  function registerTeam (message) {
    ...
    log.info('New team registered: ' + JSON.stringify(chores))
  ...
  controller
    .hears('show chores', atBot, function (bot, message) {
      ...
      if (!chores || !chores.chores.length) {
        log.error('No chore object found for resource %s', bot.config.resourceID)
      ...
      bot.startConversation(message, function (err, convo) {
        ...
        convo.ask(dictionary('CHORES_WHO'), function (resp, convo) {
          ...
          try {
            ...
            log.error('Error assigning chore', e)
            convo.say(dictionary('CHORES_ASSIGN_ERROR', { member: member, chore: chore.name }))
            convo.next()
            return
          }

          log.info('assigned chore %s to %s ', chore.name, member)
    ...
    .hears('done with chore', atBot, function (bot, message) {
      bot.startConversation(message, function (err, convo) {
        ...
        convo.ask(dictionary('CHORES_WHO'), function (resp, convo) {
          ...
          if (!chores.isMember(member)) {
            log.info('%s is not a member', member)
            if (++askedCounter > 3) {
              convo.say(dictionary('CHORES_NOT_A_MEMBER_MAX'))
              return convo.next()
            }
